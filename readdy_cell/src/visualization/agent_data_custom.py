...
from simulariumio import DimensionData, DisplayData
...
class AgentData:
    ...
    def __init__(
        ...
        (shape = [timesteps, agents, subpoints]) (optional)
            ...
            for each agent at each timestep. T/home/earkfeld/PycharmProjects/mitosim/experiments/faster_simulariumhese values are
    ...
    def _get_buffer_data_dimensions(buffer_data: Dict[str, Any]) -> DimensionData:
        ...
        for time_index in range(result.total_steps):
            ...
            buffer = bundle_data[time_index]["/home/earkfeld/PycharmProjects/mitosim/experiments/faster_simulariumdata"]
            buffer_index = 0
            agents = 0
    ...
    def from_buffer_data(
        ...
        times = np.array([bundle["time"] for bundle in bundle_data])
        agent_data.times[:] = times

        frame_data_list = [bundle["data"] for bundle in bundle_data]
        frame_data_flattened = np.concatenate(frame_data_list)

        # Define the offset indices
        indices = np.arange(0, len(frame_data_flattened), V1_SPATIAL_BUFFER_STRUCT.NSP_INDEX)
        time_indices = np.repeat(np.arange(dimensions.total_steps), len(indices) // dimensions.total_steps)

        # Vectorize the extraction of data for each agent
        agent_data.viz_types[time_indices] = frame_data_flattened[
            indices + V1_SPATIAL_BUFFER_STRUCT.VIZ_TYPE_INDEX].reshape(dimensions.total_steps, dimensions.max_agents)
        agent_data.unique_ids[time_indices] = frame_data_flattened[
            indices + V1_SPATIAL_BUFFER_STRUCT.UID_INDEX].reshape(dimensions.total_steps, dimensions.max_agents)
        type_ids[time_indices] = frame_data_flattened[indices + V1_SPATIAL_BUFFER_STRUCT.TID_INDEX].reshape(
            dimensions.total_steps, dimensions.max_agents)

        # Positions and rotations
        agent_data.positions[time_indices] = frame_data_flattened[
            np.add.outer(indices, [V1_SPATIAL_BUFFER_STRUCT.POSX_INDEX, V1_SPATIAL_BUFFER_STRUCT.POSY_INDEX,
                                   V1_SPATIAL_BUFFER_STRUCT.POSZ_INDEX])
        ].reshape(dimensions.total_steps, dimensions.max_agents, 3)

        agent_data.rotations[time_indices] = frame_data_flattened[
            np.add.outer(indices, [V1_SPATIAL_BUFFER_STRUCT.ROTX_INDEX, V1_SPATIAL_BUFFER_STRUCT.ROTY_INDEX,
                                   V1_SPATIAL_BUFFER_STRUCT.ROTZ_INDEX])
        ].reshape(dimensions.total_steps, dimensions.max_agents, 3)

        # Radii
        agent_data.radii[time_indices] = frame_data_flattened[indices + V1_SPATIAL_BUFFER_STRUCT.R_INDEX].reshape(
            dimensions.total_steps, dimensions.max_agents)

        # Subpoints
        if dimensions.max_subpoints > 0:
            n_subpoints = frame_data_flattened[indices + V1_SPATIAL_BUFFER_STRUCT.NSP_INDEX].reshape(
                dimensions.total_steps, dimensions.max_agents).astype(int)
            agent_data.n_subpoints[time_indices] = n_subpoints

            subpoint_indices = np.add.outer(indices, np.arange(1, dimensions.max_subpoints + 1))
            valid_indices = (n_subpoints > 0).reshape(-1)

            # Use fancy indexing to extract subpoints
            agent_data.subpoints[time_indices[valid_indices]] = frame_data_flattened[subpoint_indices[valid_indices]]

        # Number of agents at each timestep
        agent_data.n_agents[:] = np.sum(agent_data.unique_ids != 0, axis=1)

        # Get type names and display data
        type_names = AgentData.get_type_names(type_ids, buffer_data["trajectoryInfo"]["typeMapping"])
    ...
    def from_dimensions(
        ...
        """
        Sets up AgentData arrays with the required dimensions
        """
        return cls(
    ...
    def check_increase_buffer_size(
        ...
        if axis == 0:  # time dimension
            required_steps = next_index + 1
            if required_steps > current_dimensions.total_steps:
                # Calculate the necessary increase in buffer size
                additional_steps = (
                                           required_steps - current_dimensions.total_steps + buffer_size_inc.total_steps - 1) // buffer_size_inc.total_steps
                new_total_steps = current_dimensions.total_steps + additional_steps * buffer_size_inc.total_steps
                result = result.get_copy_with_increased_buffer_size(
                    DimensionData(
                        total_steps=new_total_steps - current_dimensions.total_steps,
                        max_agents=0,
                    ),
                    axis,
                )
            ...
            required_agents = next_index + 1
            if required_agents > current_dimensions.max_agents:
                # Calculate the necessary increase in buffer size
                additional_agents = (
                                            required_agents - current_dimensions.max_agents + buffer_size_inc.max_agents - 1) // buffer_size_inc.max_agents
                new_max_agents = current_dimensions.max_agents + additional_agents * buffer_size_inc.max_agents
                result = result.get_copy_with_increased_buffer_size(
                    DimensionData(
                        total_steps=0,
                        max_agents=new_max_agents - current_dimensions.max_agents,
                    ),
                    axis,
                )

        elif axis == 2:  # subpoints dimension
            required_subpoints = next_index + 1
            if required_subpoints > current_dimensions.max_subpoints:
                # Calculate the necessary increase in buffer size
                additional_subpoints = (
                                               required_subpoints - current_dimensions.max_subpoints + buffer_size_inc.max_subpoints - 1) // buffer_size_inc.max_subpoints
                new_max_subpoints = current_dimensions.max_subpoints + additional_subpoints * buffer_size_inc.max_subpoints
                result = result.get_copy_with_increased_buffer_size(
                    DimensionData(
                        total_steps=0,
                        max_agents=0,
                        max_subpoints=new_max_subpoints - current_dimensions.max_subpoints,
                    ),
                    axis,
                )
