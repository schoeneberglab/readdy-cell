...
from src.utils.readdy_utils import ReaddyUtils
...
class MitochondrialDynamics:
    ...
    def fission1_rate_function(self, topology):
        n = len(topology.get_graph().get_vertices(vertex_type="mitochondria#"))
        return self.parameters["fission_rate"] * n
    ...
    def reaction_function_fission1_with_at(self, topology):
        ...
        vs_mito = topology.get_graph().get_vertices(vertex_type="mitochondria#", exact_match=False)
        if len(vs_mito) == 1:
            return recipe
        else:
    ...
    def reaction_function_fission2_with_at(self, topology):
        ...
        recipe = readdy.StructuralReactionRecipe(topology)
        vs_mito_im = topology.get_graph().get_vertices(vertex_type="mitochondria#Fission-IM1", exact_match=True)
        vs_motors = topology.get_graph().get_vertices(vertex_type="motor#", exact_match=False)
        ...
        for top_pair in reactive_topology_pairs:
            for particle_pair in reactive_particle_pairs:
                ...
                system.topologies.add_spatial_reaction(rxn_descriptor,
                                                       rate=self.parameters["fusion_rate"],
                                                       radius=self.parameters["fusion_radius"])
                i += 1
    ...
    def reaction_function_fusion2_with_at(self, topology):
        ...
        if len(topology.particles) == 1:
            ...
            try:
                v_motor = topology.get_graph().get_vertices(vertex_type="motor#", exact_match=False)[0]
                if v_motor:
            ...
            vs_mito_fusion = topology.get_graph().get_vertices(vertex_type="mitochondria#Fusion-IM1", exact_match=True)
            vs_motors_total = topology.get_graph().get_vertices(vertex_type="motor#", exact_match=False)
    ...
    def reaction_function_fusion2_without_at(topology):
        """ 2nd reaction in fusion mechanism """
        recipe = readdy.StructuralReactionRecipe(topology)
        vs_mito_fusion = topology.get_graph().get_vertices(vertex_type="Fusion-#IM1", exact_match=False)
